...
import { deleteTechnology, saveTechnology } from "./actions";
...
export function TechnologyList({
  ...
  const grouped = useMemo(() => {
    return TECHNOLOGY_CATEGORY_ORDER.map((category) => ({
      category,
      items: technologies.filter((tech) => (tech.category ?? "other") === category),
...
function TechnologyDialog({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const result = await saveTechnology(projectId, {
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                setForm((current) => ({
                  ...current,
                  name,
                  category: categoryTouched ? current.category : guessTechnologyCategory(name),
                }));
              }}
            />
          </div>
