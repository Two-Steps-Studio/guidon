        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            >
              ...
              {githubConfigured ? t("configured") : t("notConfigured")}
            </Badge>
          </CardHeader>
          <CardContent className="space-y-2">
            <MaskedRow label="GITHUB_APP_CLIENT_ID" isSet={githubClientId} notSetLabel={t("notSet")} />
            <MaskedRow label="GITHUB_APP_CLIENT_SECRET" isSet={githubClientSecret} notSetLabel={t("notSet")} />
            <MaskedRow label="GITHUB_APP_SLUG" isSet={githubAppSlug} notSetLabel={t("notSet")} />
          </CardContent>
        </Card>
      </section>
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y divide-border">
                <tr className="[&>td]:px-4 [&>td]:py-3">
                  <td className="text-muted-foreground">{t("storageProviderRow")}</td>
                  <td className="font-medium capitalize">{storageProvider ?? t("invalidConfiguration")}</td>
                </tr>
                <tr className="[&>td]:px-4 [&>td]:py-3">
                  <td className="text-muted-foreground">{t("aiProviderRow")}</td>
                  <td className="font-medium capitalize">{aiProvider ?? t("notConfigured")}</td>
                </tr>
                <tr className="[&>td]:px-4 [&>td]:py-3">
                  <td className="text-muted-foreground">{t("authProvidersRow")}</td>
                  <td className="font-medium">
                    {authProviders.length > 0 ? authProviders.join(", ") : t("passwordOnly")}
                  </td>
                </tr>
                <tr className="[&>td]:px-4 [&>td]:py-3">
                  <td className="text-muted-foreground">{t("githubIntegrationRow")}</td>
                  <td className="font-medium">{githubConfigured ? t("configured") : t("notConfigured")}</td>
                </tr>
              </tbody>
            </table>
          </CardContent>
