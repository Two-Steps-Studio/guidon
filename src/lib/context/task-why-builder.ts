...
/**
 * Session-free core of getTaskWhyContext (src/lib/context/task-why.ts). Lives
 * in its own module, NOT in the "use server" file, on purpose: every export of
 * a "use server" file is a publicly callable Server Action, and this function
 * takes a caller-supplied `userId` and runs queries as that user - exposing it
 * as an action would let any client read another user's data. Callers must
 * have already authenticated `userId` (session via getProjectAccess, or an API
 * key via authenticateApiKey) - this function does no access check of its own
 * beyond RLS on every query.
 *
 * `getSupabase` supplies the hosted-mode (RLS-enforced) Supabase client: the
 * cookie-scoped createClient() for the UI, getApiUserClient(userId) for API
 * keys. Unused in self-hosted mode (withUser(userId, ...) carries identity).
 */
...
export async function buildTaskWhyContext(
  userId: string,
  projectId: string,
  taskId: string,
  getSupabase: SupabaseClientFactory
): Promise<TaskWhyContext> {
  ...
  if (hasDirectDatabase()) {
    try {
      ...
      if (decisionId) {
        const decisionRes = await withUser(userId, ({ query }) =>
          query(
  ...
  if (hasDirectDatabase()) {
    ...
    const withUserFetch = (table: string, columns: string, ids: Set<string> | undefined) =>
      withUser(userId, ({ query }) => fetchMapLocal(query, table, columns, ids));
