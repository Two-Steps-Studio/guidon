...
# Sentry's build plugin only needs these to upload source maps - optional,
# and unrelated to whether error tracking itself is enabled at runtime
# (that's SENTRY_DSN/NEXT_PUBLIC_SENTRY_DSN, read at runtime instead).
ARG SENTRY_ORG
ARG SENTRY_PROJECT
